...
async def lifespan(app: FastAPI):
    ...
    logger.info("PreSaleAgent 已就绪")
    yield

    await _monitor.stop()
    if _memory is not None:
        await _memory.close()
    logger.info("PreSaleAgent 已关闭")


# ── FastAPI ───────────────────────────────────────────────────────────────────
app = FastAPI(
    title="PreSaleAgent 售前导购",
    version="2.0.0",
    lifespan=lifespan,
    docs_url="/docs",
)
...
async def chat(req: ChatRequest):
    ...
    return ChatResponse(
        ...
        knowledge_used=bool({"search_knowledge_base", "search_product_catalog"} & set(result.tools_used)),
        entities=intent_result.entities,
...
async def _cli():
    print(BANNER)
    print("PreSaleAgent CLI — 输入 quit 退出\n")
    ...
    while True:
        ...
        print(f"\nPreSaleAgent [{result.agent_type.value}]: {result.response}\n")
