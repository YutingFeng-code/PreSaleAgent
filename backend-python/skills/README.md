# PreSaleAgent Skills 文档

PreSaleAgent 启动时会从 `ECHOMIND_SKILLS_DIR` 读取 Skills，并在匹配用户请求时注入到对应 Agent 的 system prompt。Skills 适合维护售前业务规范、推荐话术、规格解释、库存边界、升级规则和禁止事项。
...
skills/product_advisor/SKILL.md           # 售前导购：商品对比、推荐、规格、库存和优惠
```
