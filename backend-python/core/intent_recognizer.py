...
class IntentCategory(Enum):
    ...
    PRODUCT_RECOMMEND = "product_recommend"  # 导购推荐
    SPEC_INQUIRY = "spec_inquiry"  # 规格参数
    AVAILABILITY = "availability"  # 库存/发货时效
    PRICE_PROMOTION = "price_promotion"  # 价格/优惠
    OTHER      = "other"
...
_TEMPLATES: Dict[IntentCategory, List[str]] = {
    ...
    IntentCategory.PRODUCT_COMPARE: ["A 和 B 哪个好？", "比较这两款商品", "两款产品有什么区别"],
    IntentCategory.PRODUCT_RECOMMEND: ["给女朋友送什么礼物？", "预算 500 怎么选", "推荐一款适合通勤的产品"],
    IntentCategory.SPEC_INQUIRY: ["这款支持 NFC 吗？", "续航时间是多少", "商品的尺寸和重量是多少"],
    IntentCategory.AVAILABILITY: ["这款有现货吗？", "什么时候发货", "几天能送到"],
    IntentCategory.PRICE_PROMOTION: ["现在多少钱？", "最近有什么优惠", "可以使用优惠券吗"],
}
...
_INTENT_GROUPS: Dict[IntentCategory, IntentCategory] = {
    ...
    IntentCategory.PRODUCT_COMPARE: IntentCategory.QUERY,
    IntentCategory.PRODUCT_RECOMMEND: IntentCategory.QUERY,
    IntentCategory.SPEC_INQUIRY: IntentCategory.QUERY,
    IntentCategory.AVAILABILITY: IntentCategory.QUERY,
    IntentCategory.PRICE_PROMOTION: IntentCategory.QUERY,
}
...
可选意图: {", ".join(c.value for c in IntentCategory)}"""
    ...
    def _pattern_recognize(self, message: str) -> Dict[str, Any]:
        ...
        specific_patterns = {
            ...
            IntentCategory.PRODUCT_COMPARE: ["哪个更好", "哪个好", "对比", "区别", "比较", "compare"],
            IntentCategory.PRODUCT_RECOMMEND: ["推荐", "适合", "送给", "礼物", "预算", "recommend"],
            IntentCategory.SPEC_INQUIRY: ["规格", "参数", "支持", "功能", "续航", "尺寸", "重量", "spec"],
            IntentCategory.AVAILABILITY: ["现货", "库存", "有货", "发货", "几天到", "到货", "availability", "shipping"],
            IntentCategory.PRICE_PROMOTION: ["价格", "多少钱", "优惠", "促销", "优惠券", "折扣", "price", "promotion"],
        }
        generic_patterns = {
    ...
    def _extract_entities(self, message: str) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "product": self._unique(re.findall(r"(?:商品|产品|型号|model)\s*[:：]?\s*([\w一-龥][\w一-龥 -]{1,40})", message, re.I)),
            ...
            "budget": self._unique(re.findall(r"(?:预算|budget)\s*(?:约|为|是|在)?\s*(\d+(?:\.\d{1,2})?)\s*(?:元|块|rmb|cny|美元)?", message, re.I)),
            "scenario": self._unique(re.findall(r"(?:适合|用于|用来|场景)\s*([一-龥A-Za-z0-9][一-龥A-Za-z0-9 -]{1,30})", message)),
            "recipient": self._unique(re.findall(r"(?:送给|给)\s*([一-龥A-Za-z]{1,12})", message)),
            "feature": self._unique(re.findall(r"(?:支持|需要|关注|功能)\s*([一-龥A-Za-z0-9][一-龥A-Za-z0-9 -]{1,24})", message)),
            "quantity": self._unique(re.findall(r"(\d+)\s*(?:件|个|台|套)", message)),
            "location": self._unique(re.findall(r"(?:送到|配送到|地址)\s*([一-龥A-Za-z0-9][一-龥A-Za-z0-9 -]{1,30})", message)),
