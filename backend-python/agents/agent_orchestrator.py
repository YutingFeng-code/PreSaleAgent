...
class AgentType(Enum):
    ...
    PRODUCT_ADVISOR = "product_advisor" # 售前导购
...
class GeneralAgent(BaseAgent):
    ...
    system_prompt = (
        "你是 PreSaleAgent 的通用售前接待。友好、简洁地回答用户问题。"
...
class ProductAdvisorAgent(BaseAgent):
    agent_type = AgentType.PRODUCT_ADVISOR
    profile = AgentProfile(
        role="售前导购与商品顾问",
        mission="基于商品目录、知识库和当前需求，完成商品对比、规格解释、场景推荐、礼品建议和选购引导。",
        workflow=("识别预算/场景/对象/功能", "检索商品目录和知识库", "按统一维度比较或推荐", "说明取舍与备选", "确认下一步"),
        input_contract=("用户问题", "当前对话", "用户画像", "预算与场景", "商品目录结果", "知识库上下文"),
        output_contract=("先回应核心选择问题", "给出可核验的商品事实", "说明推荐依据和取舍", "信息不足时只询问必要字段"),
        handoff_conditions=("目录和知识库均无可靠信息", "用户要求下单、改价或售后操作", "大额/企业采购需要人工报价"),
        tool_scope=("search_knowledge_base", "search_product_catalog", "inspect_request_context"),
        temperature=0.6,
        max_tokens=1200,
    )
    system_prompt = (
        "你是 EchoMind 售前导购顾问。专注商品对比、规格解释、适用场景推荐、礼品建议和选购引导。"
    ...
    def _build_role_packet(self, req: Request) -> str:
        packet = json.loads(super()._build_role_packet(req))
        packet["presales_fields"] = {
            key: req.entities.get(key, [])
            for key in ("product", "budget", "scenario", "recipient", "feature", "quantity", "location")
        }
        packet["profile_boundary"] = "历史购买记录仅供个性化推荐参考，不作为商品事实来源；当前需求优先。"
        return json.dumps(packet, ensure_ascii=False)

    def get_tools(self) -> Dict[str, AgentToolSpec]:
        tools = super().get_tools()
        tools.update(product_catalog_tools())
        tools.update(general_tools())
        return tools
...
class AgentOrchestrator:
    ...
    _INTENT_ROUTING: Dict[IntentCategory, AgentType] = {
        ...
        IntentCategory.PRODUCT_COMPARE: AgentType.PRODUCT_ADVISOR,
        IntentCategory.PRODUCT_RECOMMEND: AgentType.PRODUCT_ADVISOR,
        IntentCategory.SPEC_INQUIRY: AgentType.PRODUCT_ADVISOR,
        IntentCategory.AVAILABILITY: AgentType.PRODUCT_ADVISOR,
        IntentCategory.PRICE_PROMOTION: AgentType.PRODUCT_ADVISOR,
        # 其余意图 → GENERAL（默认）
    }
    ...
    def __init__(
        ...
        self._pool: Dict[AgentType, List[BaseAgent]] = {
            ...
            AgentType.PRODUCT_ADVISOR: [self._make_agent(ProductAdvisorAgent, client, model, skill_manager)],
    ...
    async def run(self, req: Request) -> OrchestratorResult:
        ...
        if self._needs_clarification(req):
            result = OrchestratorResult(
                ...
                response="我还不能确定您的售前需求。请补充一下是商品对比、导购推荐、规格参数、库存发货，还是价格优惠？",
                agent_type=AgentType.GENERAL,
                intent=req.intent,
                escalated=False,
    ...
    def _domain_scores(self, req: Request) -> Dict[AgentType, float]:
        ...
        scores = {
            ...
            AgentType.PRODUCT_ADVISOR: 0.0,
        }

        presales_intents = (
            IntentCategory.PRODUCT_COMPARE, IntentCategory.PRODUCT_RECOMMEND,
            IntentCategory.SPEC_INQUIRY, IntentCategory.AVAILABILITY,
            IntentCategory.PRICE_PROMOTION,
        )
        if req.intent in presales_intents:
            scores[AgentType.PRODUCT_ADVISOR] += 0.75
        ...
        presales_kws = ["商品", "产品", "推荐", "适合", "送给", "礼物", "预算", "规格", "参数", "现货", "库存", "优惠", "多少钱", "对比"]
        ...
        presales_hits = sum(1 for kw in presales_kws if kw in msg)
        ...
        scores[AgentType.PRODUCT_ADVISOR] += min(0.45, presales_hits * 0.18)
        ...
        if any(entities.get(key) for key in ("product", "budget", "scenario", "recipient", "feature")):
            scores[AgentType.PRODUCT_ADVISOR] += 0.15
    ...
    def _collaboration_targets(self, req: Request) -> List[AgentType]:
        ...
        if req.intent in (
            ...
            IntentCategory.SPEC_INQUIRY, IntentCategory.AVAILABILITY,
            IntentCategory.PRICE_PROMOTION,
        ) or any(kw in msg for kw in presales_kws):
            targets.append(AgentType.PRODUCT_ADVISOR)
