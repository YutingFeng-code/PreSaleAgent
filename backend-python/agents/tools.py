...
def product_catalog_tools() -> Dict[str, AgentToolSpec]:
    """售前商品目录工具；未配置目录 API 时返回可解释降级结果。"""
    async def search_product_catalog(req: Request, args: Dict[str, Any]) -> Dict[str, Any]:
        query = str(args.get("query") or req.message or "").strip()
        category = str(args.get("category") or "").strip()
        limit = int(args.get("limit", 5) or 5)
        limit = max(1, min(limit, 20))
        base_url = os.getenv("ECHOMIND_CATALOG_API_URL", "").strip().rstrip("/")
        if not base_url:
            return {"success": False, "available": False, "query": query,
                    "results": [], "error": "商品目录 API 未配置"}
        timeout = float(os.getenv("ECHOMIND_CATALOG_TIMEOUT_SECONDS", "3"))
        headers = {}
        token = os.getenv("ECHOMIND_CATALOG_API_TOKEN", "").strip()
        if token:
        ...
        try:
            ...
            if not isinstance(items, list):
                return {"success": False, "available": True, "query": query,
                        "results": [], "error": "商品目录响应 items 格式无效"}
            return {"success": True, "available": True, "query": query,
                    "results": items[:limit], "count": len(items[:limit])}
        except httpx.TimeoutException:
            return {"success": False, "available": False, "query": query,
                    "results": [], "error": "商品目录请求超时"}
        except (httpx.HTTPError, ValueError) as ex:
            return {"success": False, "available": False, "query": query,
                    "results": [], "error": f"商品目录不可用: {ex}"}

    return {
        "search_product_catalog": make_tool(
            "search_product_catalog",
            "搜索实时商品目录，返回名称、价格、规格、库存和配送信息；目录不可用时不得猜测商品事实。",
            {
                "query": {"type": "string", "description": "商品名称、型号或用户需求"},
                "category": {"type": "string", "description": "商品分类，可为空"},
