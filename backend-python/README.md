...
PreSaleAgent 是一个面向售前导购场景的多 Agent 智能系统。它不是单纯的聊天机器人，而是把以下能力串成闭环：

当前默认业务域为售前导购。ProductAdvisorAgent 支持商品对比、场景推荐、规格问答、库存/发货时效和价格优惠咨询；商品事实来自 RAG 知识库或可配置的商品目录 API。商品目录配置：`ECHOMIND_CATALOG_API_URL`、`ECHOMIND_CATALOG_API_TOKEN`、`ECHOMIND_CATALOG_TIMEOUT_SECONDS=3`。
