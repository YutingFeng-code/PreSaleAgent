...
public record Skill(
    ...
    /** Backward-compatible constructor for skills that predate intent mapping. */
    public Skill(String name, String description, String content, String path,
                 List<String> keywords, List<String> agents, boolean enabled) {
        this(name, description, content, path, keywords, agents, "", enabled);
    }

    public Skill {
        intent = intent == null ? "" : intent.trim().toLowerCase(Locale.ROOT).replace('-', '_').replace(' ', '_');
    }
