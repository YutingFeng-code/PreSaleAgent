...
public class KnowledgeBaseService {
    ...
    private final ProductFactChunker productFactChunker = new ProductFactChunker(splitter);

    public KnowledgeBaseService(PreSaleAgentProperties properties, ObjectProvider<VectorStore> vectorStoreProvider, ObjectMapper objectMapper) {
    ...
    public int addDocuments(List<Map<String, String>> inputDocs) {
        ...
        for (Map<String, String> input : inputDocs) {
            ...
            ProductFactChunker.ChunkingResult chunking = productFactChunker.split(title, content);
            for (int i = 0; i < chunking.chunks().size(); i++) {
                ProductFactChunker.DocumentChunk chunk = chunking.chunks().get(i);
                ...
                metadata.put("source", "presaleagent-java");
                metadata.put("chunk_type", chunking.structured() ? "product_fact" : "general");
                if (chunk.field() != null) {
                    metadata.put("fact_section", chunk.field());
                }
